...
class OAmatrixGenerator implements Runnable {
	...
	OAmatrixGenerator(String outputFileName, long time_n, List<String> object, List<String> actor, DataDAO dbCon,
	...
	public void run() {
		...
		try {
			...
			for (int i = 0; i < object.size() && !Thread.currentThread()
					.isInterrupted()/* && !TensorGenerator.exec.isShutdown() */; i++) {
				oneLineGenerate(filewriter, res, i, ressb);
				nowprogres++;
				setStatus_sb(nowprogres);
	...
	private void setStatus_sb(int nowprogres) {
		...
		status_sb.append(
				TENSOR_GEN_OUTPUT_FILE_NAME/*
											 * + outputFileName + TIME_N2 +
											 * time_n + space + nowprogres +
											 * SLASH + objectsize
											 */);
